        ...
        <div class="row">
            <div class="col-xxl">
                <div class="card mb-4">
                    ...
                    <div class="card-body">
                        <form
                            action="{{ isset($materi) ? route('materi_program.update', $materi->id) : route('materi_program.store') }}"
                            ...
                            <!-- Module Selection -->
                            <div class="row mb-3">
                                <label class="col-sm-2 col-form-label" for="product_id">Program</label>
                                <div class="col-sm-10">
                            ...
                            <!-- Tambahkan class untuk kontrol tampil/hide -->
                            <div class="row mb-3 tipe-pdf-field" style="display:none;">
                                <label class="col-sm-2 col-form-label" for="tipe_pdf">Tipe PDF</label>
                                <div class="col-sm-10">
                                    <select id="tipe_pdf" name="tipe_pdf"
                                        class="form-control select2 @error('tipe_pdf') is-invalid @enderror">
                                        <option value="">-- Pilih Tipe --</option>
                                        <option value="1"
                                            {{ old('tipe_pdf', $materi->tipe_pdf ?? '') == '1' ? 'selected' : '' }}>
                                            Bisa Download
                                        </option>
                                        <option value="2"
                                            {{ old('tipe_pdf', $materi->tipe_pdf ?? '') == '2' ? 'selected' : '' }}>
                                            Tidak Bisa Download
                                        </option>
                                    </select>
                                    @error('tipe_pdf')
                            ...
                            <div class="row justify-content-end">
                                <div class="col-sm-10">
                                    ...
                                    <a href="{{ route('materi_program.index') }}" class="btn btn-secondary">Kembali</a>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    <script>
        ...
        function initFilePond() {
            ...
            filePondInstance = FilePond.create(inputElement, {
                ...
                server: {
                    process: {
                        url: '{{ route('materi_program.upload-video-chunk') }}',
                        method: 'POST',
                        headers: {
                    ...
                    revert: {
                        url: '{{ route('materi_program.delete-video-chunk') }}',
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
        ...
        function initPdfFilePond() {
            ...
            pdfPondInstance = FilePond.create(inputElement, {
                ...
                server: {
                    process: {
                        url: '{{ route('materi_program.upload-pdf-chunk') }}',
                        method: 'POST',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
                    ...
                    revert: {
                        url: '{{ route('materi_program.delete-pdf-chunk') }}',
                        method: 'DELETE',
                        headers: {
                            'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
                        },
