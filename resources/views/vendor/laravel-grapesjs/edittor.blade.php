...
<body>
    ...
    <script src="https://unpkg.com/grapesjs-plugin-ckeditor@0.0.10/dist/grapesjs-plugin-ckeditor.min.js"></script>
    @foreach ($editorConfig->getScripts() as $script)
        <script src="{{ $script }}"></script>
    @endforeach
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Tunggu editor terinisialisasi
            if (window.editor) {
                const sm = window.editor.StyleManager;

                // Tambahkan sektor typography jika belum ada
                if (!sm.getSector('typography')) {
